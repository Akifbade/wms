...
export const Shipments: React.FC = () => {
  ...
  const loadShipments = async () => {
    try {
      ...
      const sorted = [...pinned, ...unpinned].sort((a: any, b: any) => {
        const gc = (s: any) => new Date(s.createdAt || 0).getTime(); const gd = (s: any) => getDaysStored(s);
        switch (sortBy) {
          case 'date_desc': return gc(b) - gc(a); case 'date_asc': return gc(a) - gc(b);
          case 'name_asc': return (a.clientName || '').localeCompare(b.clientName || ''); case 'name_desc': return (b.clientName || '').localeCompare(a.clientName || '');
          case 'cbm_desc': return (Number(b.cbm) || 0) - (Number(a.cbm) || 0); case 'cbm_asc': return (Number(a.cbm) || 0) - (Number(b.cbm) || 0);
          case 'duration_desc': return gd(b) - gd(a); case 'duration_asc': return gd(a) - gd(b);
          case 'pieces_desc': return (b.currentBoxCount || 0) - (a.currentBoxCount || 0); case 'pieces_asc': return (a.currentBoxCount || 0) - (b.currentBoxCount || 0);
  ...
  const handlePrintLabel = (shipment: any) => {
    ...
    w.document.write(`<html><head><title>Label - ${shipment.referenceId}</title><style>body{font-family:sans-serif;padding:20px}table{width:100%;border-collapse:collapse}td{padding:8px;border:1px solid #ccc}.label{border:2px solid #000;padding:20px;margin-bottom:20px;text-align:center}h1{font-size:24px;margin:0 0 10px}.qr{font-family:monospace;font-size:18px;margin:10px 0}.ref{font-size:14px;color:#666}</style></head><body><div class="label"><h1>📦 SHIPMENT LABEL</h1><p class="ref">Ref: ${shipment.referenceId}</p><p><strong>Client:</strong> ${shipment.clientName || 'N/A'}</p><p><strong>Company:</strong> ${shipment.companyProfile?.name || 'N/A'}</p><p><strong>Boxes:</strong> ${shipment.currentBoxCount || 0} / ${shipment.originalBoxCount || 0}</p><p><strong>Location:</strong> ${shipment.rackLocations || 'Unassigned'}</p><p><strong>Arrival:</strong> ${formatDate(shipment.arrivalDate)}</p>${shipment.qrCode ? `<p class="qr">QR: ${shipment.qrCode}</p>` : ''}<p style="margin-top:20px;font-size:10px;color:#999">Generated ${new Date().toLocaleString()}</p></div><script>window.print();window.close();</script></body></html>`); w.document.close();
  };
  const handleExportCSV = () => {
    const h = ['Reference ID', 'Client Name', 'Company', 'Status', 'Boxes', 'CBM', 'Weight', 'Location', 'Arrival Date', 'Days Stored', 'Created By'];
    const rows = shipments.map((s: any) => [s.referenceId, s.clientName, s.companyProfile?.name || '', s.status, `${s.currentBoxCount}/${s.originalBoxCount}`, s.cbm ? Number(s.cbm).toFixed(2) : '', s.weight || '', s.rackLocations || '', formatDate(s.arrivalDate || s.createdAt), getDaysStored(s), s.createdBy?.name || 'System']);
    ...
    const a = document.createElement('a'); a.href = u; a.download = `shipments_${new Date().toISOString().slice(0, 10)}.csv`; a.click(); URL.revokeObjectURL(u);
  };
  ...
  // ── PHOTO GALLERY GRID ──────────────────────
  const PhotoGalleryGrid = ({ photos, onPhotoClick }: { photos: string[]; onPhotoClick: (photos: string[], index: number) => void }) => {
    ...
    if (count === 1) {
      return (
        <div className="grid grid-cols-1 gap-2">
          <button onClick={() => onPhotoClick(photos, 0)}
            className="relative group aspect-video rounded-2xl overflow-hidden border-2 border-slate-100 hover:border-blue-400 transition-all shadow-sm hover:shadow-lg">
            ...
            <div className="absolute inset-0 flex items-center justify-center opacity-0 group-hover:opacity-100 transition-opacity">
              <div className="bg-white/90 rounded-2xl px-4 py-2 shadow-lg backdrop-blur-sm">
                <CameraIcon className="h-6 w-6 text-blue-600" />
              </div>
            </div>
            <div className="absolute bottom-2 right-2 bg-black/60 text-white text-[10px] font-bold px-2 py-1 rounded-lg backdrop-blur-sm">1/1</div>
          </button>
        </div>
      );
    }
        ...
        <div className="grid grid-cols-2 gap-2">
          {photos.map((photo, idx) => (
            <button key={idx} onClick={() => onPhotoClick(photos, idx)}
              className="relative group aspect-square rounded-xl overflow-hidden border-2 border-slate-100 hover:border-blue-400 transition-all shadow-sm hover:shadow-md">
              <img src={getBackendPhoto(photo)} alt="" className="w-full h-full object-cover group-hover:scale-110 transition-transform duration-500" />
              <div className="absolute inset-0 bg-black/0 group-hover:bg-black/30 transition-all flex items-center justify-center">
                <CameraIcon className="h-6 w-6 text-white opacity-0 group-hover:opacity-100 transition-opacity drop-shadow-lg" />
              </div>
              <div className="absolute bottom-1.5 right-1.5 bg-black/60 text-white text-[9px] font-bold px-1.5 py-0.5 rounded-lg backdrop-blur-sm">{idx + 1}/{count}</div>
            </button>
          ))}
        </div>
      );
    ...
    if (count === 3) {
      return (
        <div className="grid grid-cols-2 gap-2">
          <button onClick={() => onPhotoClick(photos, 0)}
            className="relative group row-span-2 rounded-xl overflow-hidden border-2 border-slate-100 hover:border-blue-400 transition-all shadow-sm hover:shadow-md">
            <img src={getBackendPhoto(photos[0])} alt="" className="w-full h-full object-cover group-hover:scale-110 transition-transform duration-500" />
            <div className="absolute inset-0 bg-black/0 group-hover:bg-black/30 transition-all flex items-center justify-center">
              <CameraIcon className="h-8 w-8 text-white opacity-0 group-hover:opacity-100 transition-opacity drop-shadow-lg" />
            </div>
            <div className="absolute bottom-1.5 right-1.5 bg-black/60 text-white text-[9px] font-bold px-1.5 py-0.5 rounded-lg backdrop-blur-sm">1/{count}</div>
          </button>
          <div className="grid grid-rows-2 gap-2">
            {photos.slice(1, 3).map((photo, idx) => (
              <button key={idx} onClick={() => onPhotoClick(photos, idx + 1)}
                className="relative group rounded-xl overflow-hidden border-2 border-slate-100 hover:border-blue-400 transition-all shadow-sm hover:shadow-md">
                <img src={getBackendPhoto(photo)} alt="" className="w-full h-full object-cover group-hover:scale-110 transition-transform duration-500" />
                <div className="absolute inset-0 bg-black/0 group-hover:bg-black/30 transition-all flex items-center justify-center">
                  <CameraIcon className="h-4 w-4 text-white opacity-0 group-hover:opacity-100 transition-opacity drop-shadow-lg" />
                </div>
                <div className="absolute bottom-1 right-1 bg-black/60 text-white text-[8px] font-bold px-1 py-0.5 rounded-md backdrop-blur-sm">{idx + 2}/{count}</div>
              </button>
            ))}
          </div>
        </div>
      ...
      <div className="grid grid-cols-2 gap-2">
        {photos.slice(0, 4).map((photo, idx) => {
          ...
          return (
            <button key={idx} onClick={() => onPhotoClick(photos, idx)}
              className="relative group aspect-square rounded-xl overflow-hidden border-2 border-slate-100 hover:border-blue-400 transition-all shadow-sm hover:shadow-md">
              <img src={getBackendPhoto(photo)} alt="" className={`w-full h-full object-cover ${isLast ? 'opacity-60' : ''} group-hover:scale-110 transition-transform duration-500`} />
              <div className="absolute inset-0 bg-black/0 group-hover:bg-black/30 transition-all flex items-center justify-center">
                <CameraIcon className="h-5 w-5 text-white opacity-0 group-hover:opacity-100 transition-opacity drop-shadow-lg" />
              </div>
              {isLast && (
                <div className="absolute inset-0 flex items-center justify-center bg-black/30">
                  <span className="text-white text-lg font-black drop-shadow-lg">+{count - 4}</span>
                </div>
              )}
              <div className="absolute bottom-1.5 right-1.5 bg-black/60 text-white text-[9px] font-bold px-1.5 py-0.5 rounded-lg backdrop-blur-sm">{idx + 1}/{count}</div>
            </button>
      ...
      <div className="flex gap-1.5 flex-wrap">
        {shown.map((photo, idx) => (
          <button key={idx} onClick={() => onPhotoClick(photos, idx)}
            ...
            <img src={getBackendPhoto(photo)} alt="" className="w-full h-full object-cover" />
  ...
  const HealthBadge = ({ days }: { days: number }) => {
    ...
    return <span className={`inline-flex items-center gap-1 px-2 py-0.5 rounded-full text-[10px] font-bold border shadow-sm ${meta.color} ${meta.pulse}`}><Icon className="h-3 w-3" />{meta.label}</span>;
  };

  // ── Status Badge ───────────────────────────
  const StatusBadge = ({ status }: { status: string }) => {
    const meta: Record<string, any> = { PENDING: { label: 'Pending', icon: ClockIcon, bg: 'bg-amber-100 text-amber-700 border-amber-200', dot: 'bg-amber-400' }, IN_WAREHOUSE: { label: 'Stored', icon: CheckCircleIcon, bg: 'bg-emerald-100 text-emerald-700 border-emerald-200', dot: 'bg-emerald-400' }, IN_STORAGE: { label: 'Stored', icon: CheckCircleIcon, bg: 'bg-emerald-100 text-emerald-700 border-emerald-200', dot: 'bg-emerald-400' }, ACTIVE: { label: 'Stored', icon: CheckCircleIcon, bg: 'bg-emerald-100 text-emerald-700 border-emerald-200', dot: 'bg-emerald-400' }, PARTIAL: { label: 'Partial', icon: ExclamationCircleIcon, bg: 'bg-orange-100 text-orange-700 border-orange-200', dot: 'bg-orange-400' }, RELEASED: { label: 'Released', icon: TruckIcon, bg: 'bg-blue-100 text-blue-700 border-blue-200', dot: 'bg-blue-400' } };
    const m = meta[status] || { label: status, icon: CubeIcon, bg: 'bg-gray-100 text-gray-700 border-gray-200', dot: 'bg-gray-400' }; const Icon = m.icon;
    return <span className={`inline-flex items-center gap-1 px-2.5 py-1 rounded-full text-[11px] font-bold border shadow-sm ${m.bg}`}><span className={`w-1.5 h-1.5 rounded-full ${m.dot} animate-pulse-soft`} /><Icon className="h-3 w-3" />{m.label}</span>;
  };
  ...
  const ActionBtn = ({ icon: Icon, color, onClick, title }: { icon: any; color: string; onClick: () => void; title: string }) => {
    ...
    return <button onClick={onClick} className={`p-1.5 md:p-2 rounded-xl transition-all duration-200 hover:scale-110 active:scale-90 ${colors[color]} relative overflow-hidden group/btn`} title={title}><Icon className="h-4 md:h-4.5 w-4 md:w-4.5 relative z-10" /></button>;
  };

  const AgeBar = ({ days }: { days: number }) => (
    <div className="w-full h-1 bg-slate-100 rounded-full overflow-hidden">
      <div className={`h-full rounded-full transition-all duration-1000 ease-out ${days > 60 ? 'bg-red-400' : days > 30 ? 'bg-amber-400' : 'bg-emerald-400'}`} style={{ width: `${Math.min(100, (days / 90) * 100)}%` }} />
    </div>
  );
  ...
  const InfoField = ({ icon: Icon, label, value, isBtn, onClick }: { icon: any; label: string; value: string | React.ReactNode; isBtn?: boolean; onClick?: () => void }) => (
    <div className="rounded-xl border border-slate-100 p-2.5 bg-white/60 hover:bg-white/90 transition-colors">
      ...
      {isBtn ? <button onClick={onClick} className="text-sm font-bold text-blue-600 hover:text-blue-800 hover:underline cursor-pointer transition-colors">{value}</button> : <p className="text-sm font-bold text-slate-700">{value}</p>}
    </div>
  );

  const StorageCost = ({ days }: { days: number }) => {
    if (days === 0) return null;
    return <div className="text-[10px] text-slate-400 flex items-center gap-1 mt-0.5">💰 Est. ₹{(days * storageRate).toLocaleString()} (@ ₹{storageRate}/day)</div>;
  };
  ...
  const ActivityTimeline = ({ shipmentId }: { shipmentId: string }) => {
    ...
    return <div className="space-y-2 py-1">{items.map((item: any, idx: number) => (
      <div key={idx} className="flex items-start gap-2 text-xs">
        ...
        <div className="flex-1 min-w-0"><p className="font-medium text-slate-700">{item.description}</p><p className="text-[10px] text-slate-400">{item.timestamp ? formatDate(item.timestamp) : ''}</p></div>
      </div>
    ))}</div>;
  };

  // ── SHIPMENT CARD ──────────────────────────
  const ShipmentCard = ({ shipment, index }: { shipment: any; index?: number }) => {
    ...
    return (
      <div ref={cardRef} onMouseMove={handleMouseMove} onMouseLeave={handleMouseLeave}
        className={`relative rounded-2xl border shadow-sm hover:shadow-xl transition-all duration-300 overflow-hidden group animate-fadeIn ${isReleased ? 'border-blue-200 bg-white' : days > 60 ? 'border-red-200 bg-gradient-to-br from-white to-red-50/40' : days > 30 ? 'border-amber-200 bg-gradient-to-br from-white to-amber-50/30' : 'border-slate-200 bg-white'}`}
      ...
      >
        ...
        <div className="relative z-10 p-4 md:p-5">
          ...
          <div className="flex items-start gap-3 mb-3">
            ...
            <div className="flex-1 min-w-0">
              <div className="flex items-center gap-1.5 flex-wrap">
                ...
                <h3 className="text-sm md:text-base font-bold text-slate-800 truncate max-w-[180px] md:max-w-[280px]" title={shipment.clientName}>{shipment.clientName}</h3>
              ...
              {shipment.companyProfile && (
                <div className="flex items-center gap-1.5 mt-1.5">
                  <div className="flex items-center gap-1 px-2 py-0.5 rounded-lg bg-blue-50 text-blue-700 text-[10px] font-semibold border border-blue-100"><BuildingOfficeIcon className="h-3 w-3" />{shipment.companyProfile.name}</div>
                </div>
              )}
            </div>
          </div>
          ...
          {photos.length > 0 && (
            <div className="mb-4">
              <div className="flex items-center gap-1.5 mb-2">
                <div className="p-1 bg-blue-50 rounded-lg"><CameraIcon className="h-3.5 w-3.5 text-blue-500" /></div>
                <span className="text-[11px] font-semibold text-slate-500 uppercase tracking-wider">Photos · {photos.length}</span>
              </div>
              <PhotoGalleryGrid photos={photos} onPhotoClick={handlePhotoClick} />
            </div>
          )}
          ...
          {/* Storage cost */}
          <StorageCost days={days} />

          {/* ── INLINE NOTES ── */}
          {notesEditing === shipment.id ? (
            <div className="mb-2 mt-2 p-2 bg-blue-50 rounded-xl border border-blue-100">
              <textarea value={notesText} onChange={e => setNotesText(e.target.value)} rows={2}
                className="w-full text-xs p-2 rounded-lg border border-blue-200 focus:outline-none focus:ring-2 focus:ring-blue-300 resize-none bg-white" placeholder="Add a note..." />
            ...
            <div className="mb-2 mt-1 flex items-start gap-1.5 p-2 bg-slate-50 rounded-xl border border-slate-100">
              ...
              <button onClick={() => { setNotesEditing(shipment.id); setNotesText(shipment.notes || ''); }} className="text-blue-600 hover:text-blue-800 text-[10px] font-semibold flex-shrink-0">Edit</button>
            </div>
          ) : null}
    ...
    <div className="min-h-screen bg-gradient-to-br from-slate-50 via-white to-slate-50">
          ...
          <div className={`flex items-center justify-between mb-3 md:mb-4 transition-all duration-700 ${animReady ? 'opacity-100 translate-y-0' : 'opacity-0 translate-y-6'}`}>
            <div>
              <div className="flex items-center gap-3">
                <div className={`p-2.5 rounded-2xl bg-gradient-to-br ${accentColor} shadow-lg shadow-blue-200/30 ring-1 ring-white/20`}><InboxArrowDownIcon className="h-5 w-5 md:h-6 md:w-6 text-white" /></div>
                <div><h1 className="text-xl md:text-2xl font-bold text-slate-800 tracking-tight">Shipments</h1><p className="text-xs md:text-sm text-slate-400">Manage intake, storage, and release operations</p></div>
              </div>
            </div>
              ...
              <div className="relative" ref={filterRef}>
                ...
                {filterOpen && (
                  <div className="absolute right-0 mt-1 w-48 rounded-2xl border shadow-xl z-30 backdrop-blur-xl overflow-hidden bg-white/95 border-slate-200">
                    {[{ v: 'date_desc', l: '📅 Newest' }, { v: 'date_asc', l: '📅 Oldest' }, { v: 'name_asc', l: '🔤 A-Z' }, { v: 'name_desc', l: '🔤 Z-A' }, { v: 'duration_desc', l: '⏱️ Longest' }, { v: 'duration_asc', l: '⏱️ Shortest' }, { v: 'cbm_desc', l: '📦 CBM ↓' }, { v: 'cbm_asc', l: '📦 CBM ↑' }, { v: 'pieces_desc', l: '🔢 Pieces ↓' }, { v: 'pieces_asc', l: '🔢 Pieces ↑' }].map(opt => (
                  ...
                  <div className="absolute right-0 mt-1 w-52 rounded-2xl border shadow-xl z-30 backdrop-blur-xl overflow-hidden bg-white/95 border-slate-200 p-3">
                    ...
                    <div className="space-y-1.5">{allFieldOptions.map(f => (
                      <label key={f.key} className="flex items-center gap-2 cursor-pointer">
                        <input type="checkbox" checked={columnFields.includes(f.key)} onChange={() => setColumnFields(prev => prev.includes(f.key) ? prev.filter(k => k !== f.key) : [...prev, f.key])} className="h-4 w-4 rounded border-slate-300 text-blue-600 focus:ring-blue-500" />
                        <f.icon className="h-3.5 w-3.5 text-slate-400" /><span className="text-xs text-slate-700">{f.label}</span>
                      </label>
                    ))}</div>
                  </div>
                )}
            ...
            {filtersOpen && (
              <div className="bg-white/80 backdrop-blur-sm rounded-2xl border border-slate-200 p-4 mb-3 shadow-sm animate-slideDown">
                <div className="grid grid-cols-2 md:grid-cols-5 gap-3">
                  ...
                  <div><label className="text-[10px] text-slate-500 font-semibold uppercase tracking-wider block mb-1">Days: {filterDaysMin}–{filterDaysMax === 999 ? 'Any' : filterDaysMax}</label><input type="range" min={0} max={180} value={filterDaysMin} onChange={e => setFilterDaysMin(Number(e.target.value))} className="w-full accent-blue-500" /><input type="range" min={0} max={180} value={filterDaysMax === 999 ? 180 : filterDaysMax} onChange={e => setFilterDaysMax(e.target.value === '180' ? 999 : Number(e.target.value))} className="w-full accent-blue-500" /></div>
                </div>
                <div className="flex gap-2 mt-3">
                  <button onClick={() => { setFilterDateFrom(''); setFilterDateTo(''); setFilterZone(''); setFilterCompany(''); setFilterDaysMin(0); setFilterDaysMax(999); }} className="px-4 py-1.5 text-xs font-medium text-slate-600 hover:bg-slate-100 rounded-xl transition-colors">Clear All</button>
                  <div className="flex items-center gap-1 ml-auto"><label className="text-[10px] text-slate-500 uppercase tracking-wider">Rate (₹/day):</label><input type="number" value={storageRate} onChange={e => setStorageRate(Math.max(1, Number(e.target.value)))} className="w-16 px-2 py-1 text-xs rounded-xl border border-slate-200 bg-white text-center" min={1} /></div>
                </div>
              </div>
            )}
      ...
      <div className="max-w-7xl mx-auto px-3 md:px-6 pb-24 md:pb-10">
        ...
        {viewMode === 'folders' && shipments.length > 0 && (
          <div className="space-y-3 md:space-y-4 animate-fadeIn">
            {sortedFolderNames.map((company, folderIdx) => {
              ...
              const isUnassigned = company === 'Unassigned'; const totalCbm = items.reduce((sum: number, s: any) => sum + (Number(s.cbm) || 0), 0);
              return (
                <div key={company} className={`rounded-2xl border shadow-sm overflow-hidden transition-all duration-300 ${isOpen ? 'shadow-lg' : 'hover:shadow-md'} bg-white border-slate-200`} style={{ animationDelay: `${folderIdx * 80}ms`, animationFillMode: 'both' }}>
                  <button onClick={() => toggleFolder(company)} className={`w-full flex items-center justify-between transition-all duration-200 group ${isOpen ? 'bg-gradient-to-r from-slate-50 via-white to-slate-50' : 'hover:bg-slate-50/50'}`}>
                    <div className="flex items-center gap-3 md:gap-4 p-3 md:px-5 md:py-4 min-w-0">
                      ...
                      <div className="flex-shrink-0 w-10 h-10 rounded-2xl flex items-center justify-center text-lg font-bold shadow-sm bg-gradient-to-br from-blue-100 to-indigo-100 text-blue-700">{isUnassigned ? '?' : company.charAt(0).toUpperCase()}</div>
                      <div className="text-left min-w-0">
                        <h3 className={`text-sm md:text-base font-bold transition-colors truncate ${isUnassigned ? 'text-slate-500' : 'text-slate-800 group-hover:text-blue-800'}`}>{isUnassigned ? '🚫 Unassigned' : company}</h3>
                        <p className="text-[11px] md:text-xs text-slate-400 mt-0.5">{items.length} shipment{items.length !== 1 ? 's' : ''} · {totalBoxes} piece{totalBoxes !== 1 ? 's' : ''}{totalCbm > 0 && ` · ${totalCbm.toFixed(1)} m³`}</p>
                      </div>
                    </div>
                    <div className="flex items-center gap-1.5 md:gap-2 pr-3 md:pr-5">
                      {activeItems.length > 0 && !isUnassigned && <span className="hidden sm:inline-flex items-center gap-1 px-2.5 py-1 bg-emerald-50 text-emerald-700 text-[10px] md:text-xs font-bold rounded-full border border-emerald-200"><CheckCircleIcon className="h-3 w-3" />{activeItems.length}</span>}
                      {pendingItems.length > 0 && <span className="hidden sm:inline-flex items-center gap-1 px-2.5 py-1 bg-amber-50 text-amber-700 text-[10px] md:text-xs font-bold rounded-full border border-amber-200"><ClockIcon className="h-3 w-3" />{pendingItems.length}</span>}
        ...
        {viewMode === 'table' && shipments.length > 0 && (
          <div className="space-y-2 md:space-y-3 animate-fadeIn">
            ...
            {hasMore && <div className="text-center py-4"><button onClick={() => setPage(p => p + 1)} className="px-6 py-2.5 bg-white border border-slate-200 text-slate-600 text-sm font-bold rounded-xl hover:bg-slate-50 hover:border-blue-200 transition-all shadow-sm hover:shadow-md">Show More ({shipments.length - paginatedShipments.length} remaining)</button></div>}
            {!hasMore && shipments.length > PAGE_SIZE && <div className="text-center py-3 text-xs text-slate-400">Showing all {shipments.length} shipments</div>}
          </div>
        )}
      </div>
      ...
      {selectedIds.size > 0 && (
        <div className="fixed bottom-0 left-0 right-0 bg-white/95 backdrop-blur-xl border-t border-slate-200 z-40 px-4 py-3 md:px-6 md:py-4 flex items-center justify-between animate-slideUp shadow-2xl">
          <div className="flex items-center gap-3"><div className="p-1.5 rounded-xl bg-blue-50"><CheckCircleIcon className="h-4 w-4 text-blue-600" /></div><span className="text-sm font-bold text-slate-700"><span className="text-blue-600">{selectedIds.size}</span> selected</span></div>
          <div className="flex items-center gap-2 md:gap-3">
            <button onClick={() => setSelectedIds(new Set())} className="px-3 md:px-4 py-2 text-xs md:text-sm font-medium text-slate-600 hover:text-slate-800 hover:bg-slate-100 rounded-xl transition-colors">Clear</button>
            <button onClick={handleBatchRelease} className="flex items-center gap-1.5 px-4 md:px-5 py-2 text-xs md:text-sm font-bold text-white bg-gradient-to-r from-emerald-600 to-emerald-700 hover:from-emerald-700 hover:to-emerald-800 rounded-xl shadow-lg shadow-emerald-200/30 transition-all active:scale-95"><ArrowRightOnRectangleIcon className="h-4 w-4" />Release Selected</button>
          </div>
        </div>
      )}
        ...
        <div className="fixed inset-0 z-50 flex items-center justify-center p-4 bg-black/30 backdrop-blur-sm animate-fadeIn" onClick={() => setRackPopup(null)}>
          <div className="bg-white/90 backdrop-blur-xl rounded-2xl border border-slate-200 shadow-2xl p-5 md:p-6 max-w-sm w-full mx-auto animate-scaleIn" onClick={e => e.stopPropagation()}>
            ...
            <div className="space-y-4">
              <div className="rounded-2xl p-4 border bg-gradient-to-r from-blue-50 to-blue-50/50 border-blue-100"><p className="text-[10px] text-blue-500 uppercase tracking-wider font-semibold mb-1">Rack Code</p><p className="text-lg font-bold text-blue-800">{rackPopup.rackLocations || 'N/A'}</p></div>
              ...
              {rackPopup.rackLocations && <a href={`/racks?highlight=${rackPopup.boxes?.find((b: any) => b.rackId)?.rackId || ''}`} className={`flex items-center justify-center gap-2 w-full mt-1 px-4 py-3 font-semibold rounded-2xl transition-all border bg-gradient-to-r ${accentColor} text-white shadow-md hover:shadow-lg`}><SquaresPlusIcon className="h-4 w-4" />View in Racks →</a>}
            </div>
          </div>
        </div>
      )}
