    ...
    <div className="min-h-screen bg-gray-50 p-6">
      ...
      <div className="max-w-5xl mx-auto bg-white shadow-lg rounded-lg overflow-hidden">
        ...
        <div
          ...
          style={{
            borderColor: billingSettings?.primaryColor || '#2563eb',
            backgroundColor: `${billingSettings?.primaryColor || '#2563eb'}08`,
          }}
        >
          <div className="flex items-start justify-between">
            {/* Company Info */}
            <div>
              {billingSettings?.logoUrl && (
                <img
                  src={billingSettings.logoUrl}
                  alt="Company Logo"
                  className="h-16 mb-4"
                />
              )}
          ...
          <div>
            ...
            {invoice.shipment && (
              <p className="text-gray-600 text-sm mt-2">
                Shipment: <span className="font-medium">{invoice.shipment.referenceId}</span>
              </p>
            )}
          </div>
        </div>
          ...
          <div className="mt-6 flex justify-end">
            <div className="w-80">
              <div className="flex justify-between py-2 border-b">
                ...
                <span className="text-gray-900 font-semibold">{parseFloat(invoice.subtotal).toFixed(3)} KWD</span>
              </div>
              <div className="flex justify-between py-2 border-b">
                <span className="text-gray-600">Tax</span>
                <span className="text-gray-900 font-semibold">{parseFloat(invoice.taxAmount || 0).toFixed(3)} KWD</span>
              </div>
              <div className="flex justify-between py-3 border-b-2 border-gray-300">
                <span className="text-lg font-semibold text-gray-900">Total</span>
                <span className="text-lg font-bold text-gray-900">{parseFloat(invoice.totalAmount).toFixed(3)} KWD</span>
              </div>
            </div>
          </div>
        </div>
        ...
        <div className="p-8 bg-gray-50 border-b">
          ...
          <div className="grid grid-cols-3 gap-6 mb-4">
            <div>
              ...
              <p className="text-xl font-bold text-gray-900">{parseFloat(invoice.totalAmount).toFixed(3)} KWD</p>
            </div>
            <div>
              <p className="text-sm text-gray-500">Paid Amount</p>
              <p className="text-xl font-bold text-green-600">{parseFloat(invoice.paidAmount || 0).toFixed(3)} KWD</p>
            </div>
            <div>
              <p className="text-sm text-gray-500">Balance Due</p>
              <p className="text-xl font-bold text-red-600">{balance.toFixed(3)} KWD</p>
            </div>
          </div>
