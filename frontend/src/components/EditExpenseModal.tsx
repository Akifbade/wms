...
export const EditExpenseModal: React.FC<EditExpenseModalProps> = ({
  ...
  const loadCustomFieldsWithValues = async () => {
    ...
    try {
      const fieldsResponse = await fetch('http://localhost:5000/api/custom-fields?section=EXPENSE', {
        headers: {
      ...
      if (fieldsResponse.ok) {
        ...
        setCustomFields(fields.filter((field: CustomField) => field.isActive));

        const valuesResponse = await fetch(`http://localhost:5000/api/custom-field-values/EXPENSE/${expense.id}`, {
          headers: {
            'Authorization': `Bearer ${localStorage.getItem('token')}`
          }
        });
  ...
  const handleSubmit = async (e: React.FormEvent) => {
    ...
    try {
      ...
      if (customFields.length > 0) {
        ...
        if (values.length > 0) {
          try {
            await fetch(`http://localhost:5000/api/custom-field-values/EXPENSE/${expense.id}`, {
              method: 'POST',
              headers: {
