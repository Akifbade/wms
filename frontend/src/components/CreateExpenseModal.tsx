...
export const CreateExpenseModal: React.FC<CreateExpenseModalProps> = ({
  ...
  const loadCustomFields = async () => {
    try {
      const response = await fetch('http://localhost:5000/api/custom-fields?section=EXPENSE', {
        headers: {
  ...
  const handleSubmit = async (e: React.FormEvent) => {
    ...
    try {
      ...
      if (customFields.length > 0) {
        ...
        if (values.length > 0) {
          try {
            await fetch(`http://localhost:5000/api/custom-field-values/EXPENSE/${expenseId}`, {
              method: 'POST',
              headers: {
