...
export default function EditMovingJobModal({ isOpen, onClose, onSuccess, job }: EditMovingJobModalProps) {
  ...
  const loadCustomFieldsWithValues = async () => {
    ...
    try {
      ...
      const fieldsResponse = await fetch('http://localhost:5000/api/custom-fields?section=JOB', {
        headers: {
      ...
      if (fieldsResponse.ok) {
        ...
        const valuesResponse = await fetch(`http://localhost:5000/api/custom-field-values/JOB/${job.id}`, {
          headers: {
            'Authorization': `Bearer ${localStorage.getItem('token')}`
          }
        });
  ...
  const handleSubmit = async (e: React.FormEvent) => {
    ...
    try {
      ...
      if (customFields.length > 0) {
        ...
        if (values.length > 0) {
          try {
            await fetch(`http://localhost:5000/api/custom-field-values/JOB/${job.id}`, {
              method: 'POST',
              headers: {
