                  ...
                  <div className="space-y-3">
                    {Object.entries(rackGroups).map(([rackCode, boxes]) => {
                      ...
                      return (
                        <div
                          key={rackCode}
                          className={`border-2 rounded-lg p-3 ${isUnassigned
                        ...
                        >
                          <div className="flex items-center justify-between mb-2">
                            <div>
                              <div className="flex items-center gap-2">
                                ...
                                <span className={`px-2 py-0.5 rounded-full text-xs font-semibold ${isUnassigned
                          ...
                          <div className="flex flex-wrap gap-1 mt-2">
                            {boxes.map((box: any) => (
                              <div
                                key={box.id}
                                className={`px-2 py-1 rounded text-xs font-bold font-mono ${box.status === 'IN_STORAGE' || box.status === 'RELEASED'
            ...
            <div className="mb-6 overflow-x-auto">
                ...
                <tbody>
                  {invoice.lineItems?.map((item: any, idx: number) => (
                    <tr key={idx}>
                      ...
                      <td className="text-right font-mono">{(item.amount || 0).toFixed(3)} {currency}</td>
                    </tr>
                  ))}
                  <tr className="border-t">
                    <td className="py-1.5 font-semibold">Subtotal</td>
                    <td className="text-right font-semibold font-mono">{(invoice?.subtotal || 0).toFixed(3)} {currency}</td>
                  </tr>
                  <tr>
                    <td className="py-1.5">Tax ({(((invoice?.taxAmount || 0) / (invoice?.subtotal || 1)) * 100).toFixed(1)}%)</td>
                    <td className="text-right font-mono">{(invoice?.taxAmount || 0).toFixed(3)} {currency}</td>
                  </tr>
                  <tr className="border-t-2 border-gray-800">
                    <td className="py-2 text-lg font-bold">TOTAL AMOUNT</td>
                    <td className="text-right text-lg font-bold font-mono">{(invoice?.totalAmount || 0).toFixed(3)} {currency}</td>
                  </tr>
                </tbody>
              </table>
              <div className="mt-4 bg-gray-50 p-3 rounded text-sm">
                ...
                <p>
                  ...
                  <span className={`ml-2 px-2 py-0.5 rounded text-xs font-semibold ${invoice.paymentStatus === 'PAID'
