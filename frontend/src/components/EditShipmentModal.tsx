...
export default function EditShipmentModal({ isOpen, onClose, onSuccess, shipment }: EditShipmentModalProps) {
  ...
  const loadCustomFieldsWithValues = async () => {
    try {
      ...
      const fieldsResponse = await fetch('http://localhost:5000/api/custom-fields?section=SHIPMENT', {
        headers: { 'Authorization': `Bearer ${localStorage.getItem('token')}` }
      });
      if (fieldsResponse.ok) {
        const fields = await fieldsResponse.json();
        setCustomFields(fields.filter((f: CustomField) => f.isActive));
      }

      // Load existing values for this shipment
      if (shipment?.id) {
        const valuesResponse = await fetch(`http://localhost:5000/api/custom-field-values/SHIPMENT/${shipment.id}`, {
          headers: { 'Authorization': `Bearer ${localStorage.getItem('token')}` }
        });
  ...
  const handleSubmit = async (e: React.FormEvent) => {
    ...
    try {
      ...
      if (customFields.length > 0) {
        ...
        try {
          await fetch(`http://localhost:5000/api/custom-field-values/SHIPMENT/${shipment.id}`, {
            method: 'POST',
            headers: {
