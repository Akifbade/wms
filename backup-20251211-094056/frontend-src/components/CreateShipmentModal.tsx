...
export default function CreateShipmentModal({ isOpen, onClose, onSuccess }: CreateShipmentModalProps) {
  ...
  // NEW: Auto-generate dimension rows based on pallet count
  useEffect(() => {
    const count = formData.palletCount || 0;
    if (count > 0) {
      setDimensions(prev => {
        // Count existing auto-generated pallets
        const existingPallets = prev.filter(d => d.label.startsWith('Pallet '));
        const otherDimensions = prev.filter(d => !d.label.startsWith('Pallet '));

        // If we need more pallets
        if (existingPallets.length < count) {
          const newPallets = [...existingPallets];
          for (let i = existingPallets.length; i < count; i++) {
            newPallets.push({
              label: `Pallet ${i + 1}`,
              itemType: 'PALLET',
              quantity: 1,
          ...
          return [...newPallets, ...otherDimensions];
        }
        // If we have too many pallets, remove the excess from the end
        else if (existingPallets.length > count) {
          const keptPallets = existingPallets.slice(0, count);
          return [...keptPallets, ...otherDimensions];
        }
