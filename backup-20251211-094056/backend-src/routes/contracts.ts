    ...
    try {
        ...
        if (monthlyRate !== undefined || contractStartDate !== undefined || contractEndDate !== undefined) {
            await prisma.contractHistory.create({
                data: {
                    customerPrepaidBalanceId: contractId,
                    monthlyRate: existingContract.monthlyRate,
                    contractStartDate: existingContract.contractStartDate || existingContract.validFrom || new Date(),
                    contractEndDate: existingContract.contractEndDate || existingContract.validUntil,
...
// ============================================
router.get('/analytics/:companyProfileId', authenticateToken, async (req: AuthRequest, res) => {
    try {
        const companyId = req.user!.companyId;
        const { companyProfileId } = req.params;

        // 1. Get Contract & Financials
        const contract = await prisma.customerPrepaidBalance.findFirst({
            where: { companyId, companyProfileId }
        });

        // 2. Get Shipments Stats
        ...
        const activeShipments = shipments.filter(s => s.status === 'IN_STORAGE').length;
        const pendingShipments = shipments.filter(s => s.status === 'PENDING').length;

        // Calculate Total CBM (Active only)
        const totalCBM = shipments
            .filter(s => s.status === 'IN_STORAGE')
            .reduce((sum, s) => sum + (s.volumeCBM || 0), 0);
        ...
        const totalPaidInvoices = invoices.reduce((sum, inv) => sum + parseFloat(inv.paidAmount?.toString() || '0'), 0);

        // Contract financials
        const contractPaid = contract?.totalPaid || 0;
        const contractBalance = contract?.balanceRemaining || 0;

        // Estimated Live Price (Accrued Cost for current month)
        // If fixed rate: (Rate / 30) * Days passed
        // If CBM rate: (Total CBM * Rate * Days passed)
        let estimatedLivePrice = 0;
        const now = new Date();
        const daysInMonth = new Date(now.getFullYear(), now.getMonth() + 1, 0).getDate();
        const daysPassed = now.getDate();

        if (contract && contract.monthlyRate > 0) {
            // Fixed Rate Model
            estimatedLivePrice = (contract.monthlyRate / daysInMonth) * daysPassed;
        } else {
            // Fallback to CBM Rate (assuming 0.5 KWD per CBM per day as default if not found)
            // Ideally fetch from BillingSettings
            const cbmRate = 0.5;
            estimatedLivePrice = totalCBM * cbmRate * daysPassed;
        }
        ...
        res.json({
            ...
            financials: {
                totalInvoiced,
                totalPaid: totalPaidInvoices + contractPaid,
                pendingBalance: (totalInvoiced - totalPaidInvoices) + (contract ? (contract.totalBilled - contract.totalPaid) : 0),
                walletBalance: contractBalance
            },
            liveEstimates: {
                estimatedLivePrice: Math.round(estimatedLivePrice * 100) / 100,
                daysPassed,
                daysInMonth
            },
            contract: contract ? {
