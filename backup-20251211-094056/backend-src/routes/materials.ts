...
async function handleCreateReturn(req: AuthRequest, res: any) {
  try {
    ...
    if (parsedQuantityGood > 0 && rackId) {
      ...
      await prisma.rackStockLevel.upsert({
        ...
        create: { materialId, rackId, quantity: parsedQuantityGood, companyId, stockBatchId: uniqueBatchId },
        update: { quantity: { increment: parsedQuantityGood } },
      });

      const material = await prisma.packingMaterial.findUnique({ where: { id: materialId } });
      await prisma.packingMaterial.update({ where: { id: materialId }, data: { totalQuantity: (material?.totalQuantity || 0) + parsedQuantityGood } });
