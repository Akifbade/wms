...
router.get('/balance/:companyProfileId', authenticateToken, async (req: AuthRequest, res) => {
    try {
        const companyId = req.user!.companyId;
...
router.post('/balance', authenticateToken, authorizeRoles('ADMIN', 'MANAGER'), async (req: AuthRequest, res) => {
    try {
        const companyId = req.user!.companyId;
        const {
            companyProfileId,
            ...
            validUntil,
            notes,
            extendDays // NEW: Option to extend contract days
        } = req.body;
        ...
        if (extendDays && existingBalance) {
            ...
            // Log history for extension
            await prisma.contractHistory.create({
                data: {
                    customerPrepaidBalanceId: existingBalance.id,
                    monthlyRate: existingBalance.monthlyRate,
                    contractStartDate: existingBalance.contractStartDate || existingBalance.validFrom || new Date(),
                    contractEndDate: existingBalance.contractEndDate || existingBalance.validUntil,
                    reason: `Payment Extension (+${extendDays} days)`,
        ...
        if (existingBalance) {
            ...
            if (monthlyRate) updateData.monthlyRate = monthlyRate;
            if (newEndDate) {
...
router.get('/transactions/:companyProfileId', authenticateToken, async (req: AuthRequest, res) => {
    try {
        const companyId = req.user!.companyId;
        const { companyProfileId } = req.params;
...
router.post('/adjust', authenticateToken, authorizeRoles('ADMIN'), async (req: AuthRequest, res) => {
    try {
        const companyId = req.user!.companyId;
        const {
            companyProfileId,
...
router.put('/balance/:prepaidBalanceId', authenticateToken, authorizeRoles('ADMIN', 'MANAGER'), async (req: AuthRequest, res) => {
    try {
        const companyId = req.user!.companyId;
...
router.get('/check-validity/:companyProfileId', authenticateToken, async (req: AuthRequest, res) => {
    try {
        const companyId = req.user!.companyId;
        const { companyProfileId } = req.params;
