...
router.get('/', async (req: AuthRequest, res: Response) => {
  try {
    ...
    // Add computed box counts to each shipment
...
router.post('/', authorizeRoles('ADMIN', 'MANAGER'), async (req: AuthRequest, res: Response) => {
  try {
    ...
    // Generate master QR code for shipment
    const masterQR = `QR-SH-${Date.now()}`;
    ...
    const shipment = await prisma.shipment.create({
      data: {
        ...
        status: 'ACTIVE', // ✅ Schema allows: ACTIVE, PARTIAL, RELEASED
        createdBy: userId,
      },
      include: { rack: true },
    });
    ...
    for (let i = 1; i <= totalBoxCount; i++) {
      boxesToCreate.push({
        ...
        status: 'IN_STORAGE', // ✅ Always IN_STORAGE - ready to release
...
router.post('/:id/release-boxes', authorizeRoles('ADMIN', 'MANAGER'), async (req: AuthRequest, res: Response) => {
  try {
    const { id } = req.params;
    const { boxNumbers, releaseAll } = req.body; // boxNumbers = [1,2,3] or releaseAll = true
    const companyId = req.user!.companyId;
