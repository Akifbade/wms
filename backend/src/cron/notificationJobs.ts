...
import { emailTemplates, sendEmail, sendNotification } from '../services/emailService';
    ...
    for (const company of companies) {
      ...
      if (expiringSettings?.isEnabled) {
        ...
        for (const contract of expiringContracts) {
          ...
          await sendNotification(company.id, 'CONTRACT_EXPIRING', {
      ...
      if (expiredSettings?.isEnabled) {
        ...
        for (const contract of expiredContracts) {
          await sendNotification(company.id, 'CONTRACT_EXPIRED', {
            customerName: contract.companyProfile.name,
            contractEndDate: contract.contractEndDate!.toLocaleDateString(),
            monthlyRate: contract.monthlyRate,
            currency: company.currency,
...
export const startStorageAlertJob = () => {
    ...
    for (const company of companies) {
      ...
      for (const shipment of oldShipments) {
        const daysStored = Math.ceil(
          (new Date().getTime() - shipment.arrivalDate.getTime()) / (1000 * 60 * 60 * 24)
        );

        await sendNotification(company.id, 'STORAGE_ALERT', {
...
export const startJobCompletionApprovalReminderJob = () => {
    ...
    for (const company of companies) {
      ...
      for (const approval of pending) {
        ...
        if (notifyEmails.length > 0) {
          const template = emailTemplates.jobCompletionApprovalReminder({
          ...
          await sendNotification(company.id, 'JOB_COMPLETION_APPROVAL_REMINDER', {
            jobCode: approval.job?.jobCode || 'Unknown',
            customerName: approval.job?.clientName || 'Unknown',
            completedAt: approval.requestedAt ? new Date(approval.requestedAt).toLocaleString() : new Date().toLocaleString(),
            companyName: company.name,
  ...
  try {
    ...
    for (const company of companies) {
      ...
      for (const material of actualLowStock) {
        await sendNotification(company.id, 'LOW_STOCK_ALERT', {
...
export const startAllNotificationJobs = () => {
  ...
  startJobCompletionApprovalReminderJob();
