 ...
 */
async function checkPermission(userId, companyId, role, resource, action) {
    try {
        // Get the permission
 ...
 */
async function getUserPermissions(userId, companyId, role) {
    try {
...
function requirePermission(resource, action) {
    return async (req, res, next) => {
        try {
            ...
            // Check permission
            const hasPermission = await checkPermission(id, companyId, role, resource, action);
            if (!hasPermission) {
...
function requireAnyPermission(permissions) {
    return async (req, res, next) => {
        try {
            ...
            const checks = await Promise.all(permissions.map(p => checkPermission(id, companyId, role, p.resource, p.action)));
