...
exports.triggerManualBackupCron = triggerManualBackupCron;
...
async function ensureDirectories() {
    try {
        ...
        await promises_1.default.mkdir(AUTO_BACKUP_DIR, { recursive: true });
        await promises_1.default.mkdir(FULL_SYSTEM_DIR, { recursive: true });
        await promises_1.default.mkdir(GIT_BACKUP_DIR, { recursive: true });
...
async function createDatabaseBackup(companyId) {
    const timestamp = new Date().toISOString().replace(/[:.]/g, '-').split('T')[0];
    const filename = `DB_AUTO_${companyId}_${timestamp}_${Date.now()}.sql`;
    const filepath = path_1.default.join(AUTO_BACKUP_DIR, filename);
    try {
        const { stdout, stderr } = await execAsync(`docker exec wms-database mysqldump -u wms_user -pwmspassword123 --single-transaction --routines --triggers warehouse_wms`);
        if (stderr && !stderr.includes('Warning')) {
            throw new Error(stderr);
        }
        await promises_1.default.writeFile(filepath, stdout, 'utf-8');
...
async function createFullSystemBackup(companyId, companyName) {
    const timestamp = new Date().toISOString().replace(/[:.]/g, '-').split('T')[0];
    const filename = `FULL_SYSTEM_AUTO_${companyName.replace(/\s+/g, '_')}_${timestamp}_${Date.now()}.zip`;
    const zipPath = path_1.default.join(FULL_SYSTEM_DIR, filename);
    return new Promise(async (resolve, reject) => {
        try {
            ...
            output.on('close', () => {
                ...
                promises_1.default.unlink(dbBackupPath).catch(console.error);
            ...
            const uploadsPath = path_1.default.join(process.cwd(), 'backend', 'uploads');
            try {
            ...
            const backendPath = path_1.default.join(process.cwd(), 'backend');
            ...
            // Add docker configs
            const dockerCompose = path_1.default.join(process.cwd(), 'docker-compose.yml');
            try {
                await promises_1.default.access(dockerCompose);
...
async function enforceRetentionPolicy(directory, maxBackups, retentionDays, companyId) {
    try {
        const files = await promises_1.default.readdir(directory);
        // Filter only backup files for this company
        const backupFiles = files.filter(f => f.endsWith('.zip') || f.endsWith('.sql'));
        if (backupFiles.length === 0)
            return { deleted: 0, reason: 'no-backups' };
        // Get file stats
        ...
        if (deletedFiles.length > 0) {
            // Send notification
            try {
                await (0, emailService_1.sendNotification)(companyId, 'BACKUP_RETENTION_CLEANUP', {
...
async function backupToGit(backupPath, companyId, companyName) {
    try {
        const timestamp = new Date().toISOString().replace(/[:.]/g, '-');
        const gitBackupPath = path_1.default.join(GIT_BACKUP_DIR, companyName.replace(/\s+/g, '_'));
        // Ensure git backup directory exists
        await promises_1.default.mkdir(gitBackupPath, { recursive: true });
        // Copy backup to git directory
        const destPath = path_1.default.join(gitBackupPath, path_1.default.basename(backupPath));
        await promises_1.default.copyFile(backupPath, destPath);
        ...
        // Commit backup
        await execAsync(`git add "${path_1.default.basename(backupPath)}"`, { cwd: gitBackupPath });
        await execAsync(`git commit -m "Auto backup: ${timestamp}"`, { cwd: gitBackupPath });
        console.log(`✅ Git backup committed: ${path_1.default.basename(backupPath)}`);
        // Send notification
        await (0, emailService_1.sendNotification)(companyId, 'BACKUP_GIT_SYNC', {
            backupFile: path_1.default.basename(backupPath),
...
async function runAutoBackup(companyId, companyName, settings) {
    ...
    try {
        ...
        const cleanupResult = await enforceRetentionPolicy(backupType === 'FULL_SYSTEM' ? FULL_SYSTEM_DIR : AUTO_BACKUP_DIR, maxBackups, retentionDays, companyId);
        ...
        if (settings.emailNotifications) {
            await (0, emailService_1.sendNotification)(companyId, 'BACKUP_AUTO_COMPLETED', {
        ...
        try {
            await (0, emailService_1.sendNotification)(companyId, 'BACKUP_FAILED', {
                companyName,
...
function getNextBackupTime(settings) {
...
function checkBackupSchedule(settings) {
    const frequency = settings.autoBackupFrequency || 'daily';
    const time = settings.autoBackupTime || '03:00';
    const now = new Date();
    const [hours, minutes] = time.split(':').map(Number);
...
async function triggerManualBackupCron(companyId) {
    if (companyId) {
