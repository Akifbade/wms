...
const warehouseUtils_1 = require("../utils/warehouseUtils");
...
router.post('/intake', async (req, res) => {
    try {
        ...
        const barcode = (0, warehouseUtils_1.generateWarehouseBarcode)();
        // Create warehouse data
        const warehouseData = (0, warehouseUtils_1.createWarehouseData)({
            shipper,
            consignee,
            weight,
            pieces,
        ...
        for (let i = 1; i <= pieces; i++) {
            const pieceQR = (0, warehouseUtils_1.generatePieceQR)(barcode, i);
        ...
        res.json({
            shipment: {
                ...shipment,
                warehouseData: (0, warehouseUtils_1.parseWarehouseData)(shipment.warehouseData)
            },
            boxes
        });
    }
...
router.post('/release/:shipmentId', async (req, res) => {
    try {
        const companyId = req.user.companyId;
        ...
        const charges = (0, warehouseUtils_1.calculateWarehouseCharges)(shipment.createdAt, new Date(), 1, // Using per-box charging, not weight-based
        {
        ...
        res.json({
            invoice: {
                ...invoice,
                warehouseData: (0, warehouseUtils_1.parseWarehouseData)(invoice.warehouseData)
            },
            charges
        });
    }
...
router.post('/qr-scan', async (req, res) => {
    try {
        ...
        const pieceData = (0, warehouseUtils_1.parsePieceQR)(qrCode);
        if (pieceData) {
            const shipment = await prisma.shipment.findFirst({
                where: {
        ...
        const rackCode = (0, warehouseUtils_1.parseRackQR)(qrCode);
        if (rackCode) {
        ...
        if (qrCode.startsWith('SHIPMENT_')) {
            ...
            if (shipment) {
                result = {
                    ...
                    data: {
                        ...shipment,
                        warehouseData: shipment.warehouseData ? (0, warehouseUtils_1.parseWarehouseData)(shipment.warehouseData) : {}
                    }
                };
            }
        }
