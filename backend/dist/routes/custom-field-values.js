...
router.get('/:entityType/:entityId', auth_1.authenticateToken, async (req, res) => {
    try {
...
router.post('/:entityType/:entityId', auth_1.authenticateToken, async (req, res) => {
    try {
        const { entityType, entityId } = req.params;
        const { companyId } = req.user;
        ...
        // Create new values
        const createdValues = await Promise.all(values.map(async (value) => {
            // Skip empty values
