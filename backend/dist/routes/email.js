...
router.put('/settings', auth_1.authenticateToken, async (req, res) => {
    try {
        ...
        const { provider, isEnabled, smtpHost, smtpPort, smtpSecure, smtpUser, smtpPassword, senderName, senderEmail, dailyLimit, } = req.body;
        // Prepare update data
        const updateData = {
            provider,
            isEnabled,
...
router.post('/test', auth_1.authenticateToken, async (req, res) => {
    try {
        ...
        const result = await (0, emailService_1.testEmailConfig)(emailConfig, testEmail);
        if (result.success) {
...
router.put('/notifications/:type', auth_1.authenticateToken, async (req, res) => {
    try {
        const companyId = req.user?.companyId;
        if (!companyId) {
            return res.status(401).json({ error: 'Unauthorized' });
        ...
        const { isEnabled, notifyAdmins, notifyManagers, customEmails, alertDaysBefore, recipients, } = req.body;
...
router.post('/send', auth_1.authenticateToken, async (req, res) => {
    try {
        ...
        const template = emailService_1.emailTemplates.custom({
            title: subject,
            message,
            companyName: company?.name || 'WMS',
        });
        const result = await (0, emailService_1.sendEmail)(companyId, {
