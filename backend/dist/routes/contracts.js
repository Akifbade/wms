...
// ============================================
router.get('/customer/:companyProfileId', auth_1.authenticateToken, async (req, res) => {
    try {
        const companyId = req.user.companyId;
...
router.post('/', auth_1.authenticateToken, (0, auth_1.authorizeRoles)('ADMIN', 'MANAGER'), async (req, res) => {
    try {
        const companyId = req.user.companyId;
        const { companyProfileId, monthlyRate, paymentDueDay, // Day of month payment is due (1-28)
        maxCBM, // Max CBM allowed under contract
        maxStorageDays, // Max days goods can be stored
        contractStartDate, contractEndDate, notes } = req.body;
        if (!companyProfileId) {
...
// UPDATE CONTRACT
// ============================================
router.put('/:contractId', auth_1.authenticateToken, (0, auth_1.authorizeRoles)('ADMIN', 'MANAGER'), async (req, res) => {
    try {
        const companyId = req.user.companyId;
        const { contractId } = req.params;
        const { monthlyRate, paymentDueDay, maxCBM, maxStorageDays, contractStartDate, contractEndDate, status, notes } = req.body;
...
// ============================================
router.get('/check/:companyProfileId', auth_1.authenticateToken, async (req, res) => {
    try {
        const companyId = req.user.companyId;
        const { companyProfileId } = req.params;
...
// DELETE CONTRACT
// ============================================
router.delete('/:contractId', auth_1.authenticateToken, (0, auth_1.authorizeRoles)('ADMIN'), async (req, res) => {
    try {
        const companyId = req.user.companyId;
        const { contractId } = req.params;
...
// ============================================
router.post('/record-payment', auth_1.authenticateToken, (0, auth_1.authorizeRoles)('ADMIN', 'MANAGER'), async (req, res) => {
    try {
        const companyId = req.user.companyId;
        const { contractId, amount, paymentMonth, // e.g., "2025-06" for June 2025
...
// ============================================
router.get('/payments/:contractId', auth_1.authenticateToken, async (req, res) => {
    try {
        const companyId = req.user.companyId;
        const { contractId } = req.params;
        const contract = await prisma.customerPrepaidBalance.findFirst({
...
// ============================================
router.post('/generate-invoice/:companyProfileId', auth_1.authenticateToken, (0, auth_1.authorizeRoles)('ADMIN', 'MANAGER'), async (req, res) => {
    try {
        const companyId = req.user.companyId;
        const { companyProfileId } = req.params;
...
// ============================================
router.get('/payment-history/:companyProfileId', auth_1.authenticateToken, async (req, res) => {
    try {
        const companyId = req.user.companyId;
        const { companyProfileId } = req.params;
        ...
        const paymentsWithExtension = await prisma.$queryRaw `
        ...
        const overdueInvoices = invoices.filter(inv => (inv.paymentStatus === 'PENDING' || inv.paymentStatus === 'PARTIAL') &&
...
// ============================================
router.post('/check-overdue', auth_1.authenticateToken, (0, auth_1.authorizeRoles)('ADMIN'), async (req, res) => {
    try {
        const companyId = req.user.companyId;
...
// ============================================
router.post('/reactivate/:contractId', auth_1.authenticateToken, (0, auth_1.authorizeRoles)('ADMIN', 'MANAGER'), async (req, res) => {
    try {
        const companyId = req.user.companyId;
        const { contractId } = req.params;
        const contract = await prisma.customerPrepaidBalance.findFirst({
...
// ============================================
router.post('/record-payment/:invoiceId', auth_1.authenticateToken, (0, auth_1.authorizeRoles)('ADMIN', 'MANAGER'), async (req, res) => {
    try {
        const companyId = req.user.companyId;
        const userId = req.user.id;
...
router.get('/statement/:contractId', auth_1.authenticateToken, async (req, res) => {
    try {
        const companyId = req.user.companyId;
        const { contractId } = req.params;
