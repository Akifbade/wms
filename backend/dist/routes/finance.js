...
router.get('/top-customers', auth_1.authenticateToken, async (req, res) => {
    try {
        const companyId = getCompanyId(req);
        const { startDate, endDate } = req.query;
        const dateFilter = {};
...
router.get('/inventory', auth_1.authenticateToken, async (req, res) => {
    try {
        const companyId = getCompanyId(req);
        // Fetch all materials
        ...
        const damageStats = {};
        damages.forEach(d => {
            if (!damageStats[d.materialId])
                damageStats[d.materialId] = { qty: 0, cost: 0 };
