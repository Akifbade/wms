...
exports.sendShipmentCreatedNotification = exports.shipmentCreatedTemplate = exports.sendReleaseNotification = exports.testEmailConfig = exports.sendNotification = exports.emailTemplates = exports.getNotificationRecipients = exports.sendEmail = exports.getCompanyEmailSettings = exports.createGmailTransporter = void 0;
...
const createGmailTransporter = (email, appPassword) => {
    return nodemailer_1.default.createTransport({
        service: 'gmail',
        auth: {
            user: email,
...
exports.getCompanyEmailSettings = getCompanyEmailSettings;
// Send email function
const sendEmail = async (companyId, options) => {
    try {
        // Get company email settings
        const settings = await (0, exports.getCompanyEmailSettings)(companyId);
        ...
        if (settings.provider === 'gmail') {
            transporter = (0, exports.createGmailTransporter)(settings.smtpUser, settings.smtpPassword);
        }
        else {
...
// Log email activity
const logEmailSent = async (companyId, recipients, subject, status, messageId, error) => {
    try {
...
const getNotificationRecipients = async (companyId, notificationType) => {
    try {
      ...
      <div style="font-family: Arial, sans-serif; max-width: 700px; margin: 0 auto; padding: 20px;">
        ...
        <div style="background: #f9fafb; padding: 20px; border: 1px solid #e5e7eb; border-top: none; border-radius: 0 0 10px 10px;">
          ...
          <div style="display: grid; grid-template-columns: repeat(auto-fit, minmax(200px, 1fr)); gap: 10px; margin-bottom: 20px;">
            ${data.physicalReports.map((report, idx) => `
              <div style="border: 1px solid #e5e7eb; border-radius: 8px; padding: 10px; background: white;">
                <img src="cid:physicalReport${idx}" alt="Physical Report ${idx + 1}" style="width: 100%; height: auto; border-radius: 4px;" />
                <p style="text-align: center; margin: 5px 0 0; font-size: 12px; color: #6b7280;">Report ${idx + 1}</p>
              </div>
            `).join('')}
          </div>
          ` : ''}
...
// ============================================
const sendNotification = async (companyId, type, templateData, attachments) => {
    try {
        // Get recipients for this notification type
        const recipients = await (0, exports.getNotificationRecipients)(companyId, type);
        ...
        switch (type) {
            ...
            case 'SHIPMENT_CREATED':
                template = exports.emailTemplates.shipmentIntake(templateData);
                break;
            case 'SHIPMENT_RELEASED':
            case 'SHIPMENT_PARTIAL_RELEASE':
                template = exports.emailTemplates.shipmentReleased(templateData);
                break;
            case 'STORAGE_ALERT':
                template = exports.emailTemplates.storageAlert(templateData);
                break;
            // Contract Events
            case 'CONTRACT_NEW':
                template = exports.emailTemplates.contractNew(templateData);
                break;
            case 'CONTRACT_EXPIRING':
                template = exports.emailTemplates.contractExpiring(templateData);
                break;
            case 'CONTRACT_EXPIRED':
                template = exports.emailTemplates.contractExpired(templateData);
                break;
            case 'CONTRACT_RENEWED':
                template = exports.emailTemplates.contractNew({ ...templateData, title: '🔄 Contract Renewed' });
                break;
            // Moving Job Events
            case 'MOVING_JOB_CREATED':
            case 'MOVING_JOB_ASSIGNED':
            case 'MOVING_JOB_STARTED':
                template = exports.emailTemplates.movingJobCreated(templateData);
                break;
            case 'MOVING_JOB_COMPLETED':
                template = exports.emailTemplates.movingJobCompleted(templateData);
                break;
            case 'JOB_COMPLETION_APPROVAL_REQUEST':
                template = exports.emailTemplates.jobCompletionApprovalRequest(templateData);
                break;
            case 'JOB_COMPLETION_APPROVAL_REMINDER':
                template = exports.emailTemplates.jobCompletionApprovalReminder(templateData);
                break;
            case 'JOB_COMPLETION_REJECTED':
                template = exports.emailTemplates.jobCompletionRejected(templateData);
                break;
            // Billing & Payments
            case 'INVOICE_CREATED':
            case 'INVOICE_OVERDUE':
                template = exports.emailTemplates.invoiceCreated(templateData);
                break;
            case 'PAYMENT_RECEIVED':
            case 'PAYMENT_REMINDER':
                template = exports.emailTemplates.paymentReceived(templateData);
                break;
            // Materials & Inventory
            case 'LOW_STOCK_ALERT':
                template = exports.emailTemplates.lowStockAlert(templateData);
                break;
            case 'MATERIAL_ISSUED':
                template = exports.emailTemplates.materialIssued(templateData);
                break;
            case 'MATERIAL_RETURNED':
                template = exports.emailTemplates.materialReturned(templateData);
                break;
            case 'MATERIAL_DAMAGED':
                template = exports.emailTemplates.materialDamaged(templateData);
                break;
            case 'PURCHASE_ORDER_CREATED':
                template = exports.emailTemplates.custom({ title: '📋 Purchase Order Created', message: templateData.message || '', companyName: templateData.companyName });
                break;
            // Reports
            case 'DAILY_SUMMARY':
            case 'WEEKLY_REPORT':
                template = exports.emailTemplates.custom(templateData);
                break;
            case 'CUSTOM':
            default:
                template = exports.emailTemplates.custom(templateData);
        ...
        const emailAttachments = (attachments || []).map((att, idx) => ({
            filename: att.filename || `attachment-${idx + 1}${att.path ? path_1.default.extname(att.path) : ''}`,
            path: att.path,
            content: att.content,
            cid: att.cid, // Preserve CID for inline images (e.g., physical reports)
        }));
        // Send email
        return await (0, exports.sendEmail)(companyId, {
            to: recipients,
...
const testEmailConfig = async (config, testEmail) => {
    try {
        let transporter;
        if (config.provider === 'gmail') {
            transporter = (0, exports.createGmailTransporter)(config.smtpUser, config.smtpPassword);
        }
        else {
            transporter = createTransporter({
...
const sendReleaseNotification = async (companyId, data) => {
    try {
        // Get company email settings
        const settings = await (0, exports.getCompanyEmailSettings)(companyId);
        if (!settings || !settings.isEnabled) {
        ...
        const notificationType = data.releaseType === 'FULL' ? 'SHIPMENT_RELEASED' : 'SHIPMENT_PARTIAL_RELEASE';
        const recipients = await (0, exports.getNotificationRecipients)(companyId, notificationType);
        // Also try SHIPMENT_RELEASED if PARTIAL has no recipients
        if (recipients.length === 0 && data.releaseType === 'PARTIAL') {
            const fallbackRecipients = await (0, exports.getNotificationRecipients)(companyId, 'SHIPMENT_RELEASED');
        ...
        const template = exports.emailTemplates.shipmentReleased({
...
const shipmentCreatedTemplate = (data) => {
...
exports.shipmentCreatedTemplate = shipmentCreatedTemplate;
// Send shipment intake/created notification email
const sendShipmentCreatedNotification = async (companyId, data) => {
    try {
        // Get company email settings
        const settings = await (0, exports.getCompanyEmailSettings)(companyId);
        if (!settings || !settings.isEnabled) {
        ...
        const recipients = await (0, exports.getNotificationRecipients)(companyId, 'SHIPMENT_CREATED');
        if (recipients.length === 0) {
        ...
        const template = (0, exports.shipmentCreatedTemplate)({
...
exports.sendShipmentCreatedNotification = sendShipmentCreatedNotification;
