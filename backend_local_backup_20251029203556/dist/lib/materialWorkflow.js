...
exports.issueMaterialsToJob = issueMaterialsToJob;
exports.returnMaterialsFromJob = returnMaterialsFromJob;
exports.approveMaterialDamage = approveMaterialDamage;
const client_1 = require("@prisma/client");
const prisma = new client_1.PrismaClient();
/**
 * Issue materials to a job with proper stock deduction
 */
async function issueMaterialsToJob(data) {
 ...
 */
async function returnMaterialsFromJob(data) {
    return prisma.$transaction(async (tx) => {
        // 1. Validate job exists
        const job = await tx.movingJob.findFirst({
            where: { id: data.jobId, companyId: data.companyId },
 ...
 */
async function updateJobCosts(tx, jobId, companyId) {
    ...
    const materialsCost = job.materialIssues.reduce((sum, issue) => {
        const returned = job.materialReturns
            ...
            .reduce((rSum, r) => rSum + r.quantityGood, 0);
        const usedQuantity = issue.quantity - returned;
        return sum + (usedQuantity * issue.unitCost);
    }, 0);
    // Calculate labor cost
    const laborCost = job.assignments.reduce((sum, assignment) => {
        const hours = assignment.hoursWorked || 0;
        const rate = assignment.hourlyRate || 0;
        return sum + (hours * rate);
    }, 0);
    // Calculate damage loss
    const damageLoss = job.materialReturns.reduce((sum, ret) => {
        return sum + ret.damages.reduce((dSum, damage) => {
            // Get material cost
            const issue = job.materialIssues.find((i) => i.materialId === damage.materialId);
 ...
 */
async function approveMaterialDamage(data) {
    return prisma.$transaction(async (tx) => {
