...
exports.createShipmentWithBoxes = createShipmentWithBoxes;
exports.assignBoxesToRack = assignBoxesToRack;
exports.releaseBoxes = releaseBoxes;
 ...
 */
async function assignBoxesToRack(data) {
    return prisma.$transaction(async (tx) => {
 ...
 */
async function releaseBoxes(data) {
    return prisma.$transaction(async (tx) => {
        ...
        for (const shipmentId of shipmentIds) {
            ...
            if (shipment) {
                const remainingBoxes = shipment.boxes.filter(b => !data.boxIds.includes(b.id) && b.status !== 'RELEASED').length;
                await tx.shipment.update({
                    where: { id: shipmentId },
                    data: {
