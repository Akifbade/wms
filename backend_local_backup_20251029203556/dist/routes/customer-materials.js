...
router.get('/customers/materials', async (req, res) => {
    try {
        const companyId = req.user.companyId;
        const { search, category, status } = req.query;
        // Build where clause
        const where = { companyId, isWarehouseShipment: true };
        if (search) {
            where.OR = [
        ...
        const customers = Array.from(customerMap.values()).map(customer => ({
            ...
            items: Object.values(customer.items).map((item) => ({
                ...item,
...
router.get('/customers/:customerName/materials', async (req, res) => {
    try {
    ...
    try {
        ...
        const [totalCustomers, totalBoxes, totalShipments, categoryBreakdown] = await Promise.all([
            ...
            prisma.shipment
                ...
                .then(shipments => {
                const customers = new Set(shipments.map(s => s.customerName || s.shipper).filter(Boolean));
