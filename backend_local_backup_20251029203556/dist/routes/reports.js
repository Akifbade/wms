 ...
 */
router.post("/cost-snapshot", auth_1.authenticateToken, async (req, res) => {
    try {
 ...
 */
router.get("/job/:jobId/costs", auth_1.authenticateToken, async (req, res) => {
    try {
 ...
 */
router.get("/profitability", auth_1.authenticateToken, async (req, res) => {
    try {
        const { companyId } = req.user;
        const snapshots = await prisma.jobCostSnapshot.findMany({
            where: { companyId },
 ...
 */
router.get("/material-costs", auth_1.authenticateToken, async (req, res) => {
    try {
        const { companyId } = req.user;
