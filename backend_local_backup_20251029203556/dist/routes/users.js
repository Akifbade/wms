...
router.get('/profile/stats', async (req, res) => {
    try {
        ...
        // Get statistics
        const [rackActivities, jobAssignments,] = await Promise.all([
        ...
        const daysSinceJoined = Math.floor((new Date().getTime() - new Date(user.createdAt).getTime()) / (1000 * 60 * 60 * 24));
        res.json({
            success: true,
            data: {
                user: {
...
router.put('/:id', (0, auth_1.authorizeRoles)('ADMIN'), async (req, res) => {
    try {
        const { id } = req.params;
        const companyId = req.user.companyId;
...
router.patch('/:id/toggle', (0, auth_1.authorizeRoles)('ADMIN'), async (req, res) => {
    try {
        const { id } = req.params;
        const companyId = req.user.companyId;
        const currentUserId = req.user.id;
