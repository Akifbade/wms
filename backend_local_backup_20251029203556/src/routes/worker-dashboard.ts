...
router.get('/worker/pending', async (req: AuthRequest, res: Response) => {
  try {
      ...
      .map(shipment => {
        ...
        return {
          ...
          daysSinceArrival: Math.floor(
            (Date.now() - new Date(shipment.arrivalDate).getTime()) / (1000 * 60 * 60 * 24)
          ),
        };
      });
...
router.get('/worker/tasks', async (req: AuthRequest, res: Response) => {
  try {
    const companyId = req.user!.companyId;
...
router.post('/worker/quick-assign', async (req: AuthRequest, res: Response) => {
  try {
    const companyId = req.user!.companyId;
    const userId = req.user!.id;
    ...
    const result = await prisma.$transaction(async tx => {
      ...
      for (const shipmentId of shipmentIds) {
        ...
        if (shipment) {
          const allAssigned = shipment.boxes.every(
            b => boxIds.includes(b.id) || b.status === 'IN_STORAGE'
          );
          const someAssigned = shipment.boxes.some(
            b => boxIds.includes(b.id) || b.status === 'IN_STORAGE'
          );
...
router.post('/worker/complete/:activityId', async (req: AuthRequest, res: Response) => {
  try {
...
router.get('/worker/available-racks', async (req: AuthRequest, res: Response) => {
  try {
    const companyId = req.user!.companyId;
    ...
    const availableRacks = racks
      ...
      .filter(rack => rack.availableCapacity >= parseInt(minCapacity as string))
