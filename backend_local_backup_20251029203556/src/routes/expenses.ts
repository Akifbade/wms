...
router.get('/', async (req: AuthRequest, res) => {
  try {
    ...
    // Calculate totals
    const totalAmount = expenses.reduce((sum: number, exp: any) => sum + parseFloat(exp.amount.toString()), 0);
...
router.put('/:id', async (req: AuthRequest, res) => {
  try {
    ...
    if (receipts !== undefined) updateData.receipts = JSON.stringify(receipts);
...
router.get('/stats/summary', async (req: AuthRequest, res) => {
  try {
    ...
    // Group by status
    const byStatus = expenses.reduce((acc: any, exp: any) => {
